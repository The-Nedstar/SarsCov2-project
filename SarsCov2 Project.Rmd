...
DeltaD2Freq <- DeltaD2 %>%
  ...
  summarise(
    count = sum(Delta == TRUE),
    ND = sum(Delta == FALSE),
    frequency = count / (count + ND))
```
...
DeltaByReg <- multi_lineage_plot(DeltaD2Freq, DeltaD2Freq$date, "time", DeltaD2Freq$frequency, "Frequency", DeltaD2Freq$phecname, "region", "Delta Frequency per region", c(0.8835,0.4))
...
# Calculate predicted frequencies for smooth (continuous) dates 
  TempPred <- data.frame(date = TempDates,
    predicted_frequency = logistic_growth(as.numeric(TempDates - 
                                                     min(Temp$date)),
                                        coef(TempFit)["s"], 
                                        coef(TempFit)["f0"]))
  TempTitle <- paste(letters[n], ") fitted logistic growth for: ", Name, sep = "")
  ...
  TempPlot <- ggplot(TempComb, aes(x = date, y = frequency, colour = Data)) +
    ...
    scale_colour_jco() +
    theme_bw() +
    labs(x = "time", y = "frequency", title = TempTitle) +
    theme(legend.position = c(0.8835,0.4), 
...
```{r group plot}
png(filename = here("Figures", "BigBOI.png"), height = 25, width = 25, units = "cm", res = 400)
  # defining positions
